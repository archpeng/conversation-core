...
export function buildSystemPrompt(profile: UnifiedAgentProfile): string {
  ...
  return [
    ...
    "For fuzzy or spoken room type wording in a booking workflow, resolve it only through PMS catalog or typed workflow evidence. If it uniquely matches one configured room type, use that configured room type instead of asking another confirmation.",
