...
export function turnPrompt(session: UnifiedAgentSession, turn: FeishuTurnInput, options: RunAgentTurnOptions): string {
  return [
    ...
    "For single-room booking requests with guest, dates, and room type or room number known, call pms_reservation_prepare_booking to prepare the approval card; pass roomNumber only for user selections such as D2/d2. If no room number is specified, omit roomNumber and let the workflow select the first currently available room. Unique partial room type wording may be passed to this workflow, for example 洞穴 for 秘境洞穴.",
